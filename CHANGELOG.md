...
- **`bd dolt stop` no longer falsely reports "not running" in shared-server mode** - when running under shared-server mode (`dolt.shared-server: true` in `config.yaml` or `BEADS_DOLT_SHARED_SERVER=1`), with `dolt_mode=server` against a non-local host, or with a local server bd does not own (auto-start disabled), `bd dolt stop` previously failed with `Error: dolt server is not running` even when `bd list`, `bd dolt show`, and other CRUD commands successfully reached the server — because no bd PID file was tracked for it. It now SQL-probes the configured endpoint and prints a clear diagnostic explaining that bd cannot stop a server it did not start, with explicit manual-stop guidance (`bd dolt killall`, `lsof -i :PORT` + `kill <pid>`, or service-manager commands). bd intentionally does NOT kill processes it did not start. **JSON output shape**: in externally-managed scenarios, `bd dolt stop --json` emits `{"mode": "external", "action": "stop", "running": <bool>, "stopped": false, ...}`. ([#3687](https://github.com/gastownhall/beads/issues/3687))
