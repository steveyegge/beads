...
- **`on_complete` runtime fanout now actually runs** — the `OnCompleteSpec` was previously parsed, validated, and round-tripped through cook output, then silently dropped at pour time and never read by any executor. Closing a step with `on_complete` set produced zero spawned molecules regardless of configuration. The executor now persists `OnCompleteSpec` onto the step's issue at pour time (`metadata.on_complete`), and on close (via either `bd close` or `bd update --status closed`) reads the spec, resolves `for_each` against the step's `metadata.<path>`, substitutes `{item}` / `{item.field}` / `{index}` per iteration, and bonds the named child formula once per item. Hard-errors on missing/non-array output; per-item bond failures accumulate into `metadata.on_complete_failures`. Closes the largest declarative-fanout gap in the formula schema. ([#3782](https://github.com/gastownhall/beads/issues/3782))
