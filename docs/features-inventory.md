...
| `bd import` command availability | CLI docs vs CLI code | Secondary docs: `bd import -i ...`; code surface check for Cobra `Use: "import..."` | JSONL path, orphan mode (per docs) | Secondary docs promise import output | Would write/import issues if command existed | unknown/conflict | High | `beads/website/docs/cli-reference/sync.md:64`; `beads/website/docs/reference/troubleshooting.md:73`; `beads/cmd/bd/init.go:814`; `beads/cmd/bd/import_shared.go:69` | Primary docs were aligned to `bd export` + `bd init --from-jsonl`; secondary website docs still reference root `bd import` while Cobra root `Use:` does not expose it. |
| `bd sync` command availability | CLI docs vs CLI code | Secondary docs: `bd sync`; code shows nested sync commands plus Dolt-native push/pull | none (root command in docs) | Secondary docs promise sync behavior | Would commit/pull/push if root command existed | unknown/conflict | High | `beads/website/docs/cli-reference/sync.md:11`; `beads/website/docs/intro.md:67`; `beads/cmd/bd/backup_dolt.go:99`; `beads/cmd/bd/migrate.go:727`; `beads/cmd/bd/federation.go:38`; `beads/cmd/bd/jira.go:42` | Primary docs were aligned to Dolt-native sync (`bd dolt push/pull`); secondary website docs still reference root `bd sync`. |
| `bd merge` command availability | CLI docs vs CLI code | Secondary docs: `bd merge <src...> --into <target>`; code has `bd vc merge` and `bd merge-slot` | source/target IDs or branch names | Merge result/conflicts | Would mutate issue graph or VC state | unknown/conflict | High | `beads/website/docs/cli-reference/issues.md:221`; `beads/website/docs/reference/advanced.md:30`; `beads/cmd/bd/vc.go:30`; `beads/cmd/bd/merge_slot.go:18`; `beads/cmd/bd/duplicate.go:15` | Primary docs now prefer `duplicate`/`supersede` and `vc` surfaces; secondary website docs still advertise root `bd merge`. |
| `bd stats` command availability | Root alias for status overview | `bd stats` | optional `--json`, `--assigned`, `--no-activity` | status summary output | none (read path) | implemented | High | `beads/cmd/bd/status.go:29`; `beads/cmd/bd/status.go:31`; `beads/cmd/bd/status.go:53` | `bd stats` is a root alias of `bd status`; previous mismatch report was due to incomplete surface extraction. |
...
## Known Gaps / Indicated Features

- Root command mismatches: secondary docs (website reference pages) still advertise `bd import`, `bd sync`, and `bd merge`, while the observed Cobra `Use:` surface differs (inventory rows marked `unknown/conflict`).
- `bd stats` is a root alias for `bd status`; this is implemented in current CLI wiring.
- Windows sandbox parity: Windows auto-detect is noted as incomplete with a TODO, while manual `--sandbox` remains the documented fallback (inventory row marked `partially_implemented`).
