...
- Root command mismatches: docs advertise `bd import`, `bd sync`, `bd merge`, and `bd stats`, but the observed Cobra `Use:` surface differs (inventory rows marked `unknown/conflict`).
- Stale global flags: docs and the MCP bd client still reference `--no-auto-flush` and `--no-auto-import`, but current root persistent flags don't include them (inventory rows marked `unknown/conflict`).
