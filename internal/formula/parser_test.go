...
// hq-946577.18: Tests for decision field parsing

func TestParse_DecisionField(t *testing.T) {
	jsonData := `{
  "formula": "mol-deployment",
  "version": 1,
  "type": "workflow",
  "steps": [
    {
    ...
    {"id": "deploy", "title": "Deploy", "depends_on": ["choose-strategy"]}
  ]
}`
	p := NewParser()
	formula, err := p.Parse([]byte(jsonData))
	...
	decision := formula.Steps[0].Decision
	if decision == nil {
		t.Fatal("Steps[0].Decision is nil")
	}
	if decision.Prompt != "Select deployment strategy:" {
		t.Errorf("Decision.Prompt = %q, want 'Select deployment strategy:'", decision.Prompt)
	}
	if len(decision.Options) != 2 {
		t.Fatalf("len(Decision.Options) = %d, want 2", len(decision.Options))
	}
	if decision.Options[0].ID != "staged" {
		t.Errorf("Decision.Options[0].ID = %q, want 'staged'", decision.Options[0].ID)
	}
	if decision.Options[0].Short != "Staged" {
		t.Errorf("Decision.Options[0].Short = %q, want 'Staged'", decision.Options[0].Short)
	}
	if decision.Options[0].Label != "Staged rollout (recommended)" {
		t.Errorf("Decision.Options[0].Label = %q, want 'Staged rollout (recommended)'", decision.Options[0].Label)
	}
	if decision.Default != "staged" {
		t.Errorf("Decision.Default = %q, want 'staged'", decision.Default)
	}
	if decision.Timeout != "48h" {
		t.Errorf("Decision.Timeout = %q, want '48h'", decision.Timeout)
	}
}

func TestParse_DecisionFieldTOML(t *testing.T) {
	tomlData := `
formula = "mol-approval"
version = 1
type = "workflow"
...
`
	...
	if decision.Prompt != "Choose deployment strategy:" {
		t.Errorf("Decision.Prompt = %q, want 'Choose deployment strategy:'", decision.Prompt)
	}
	if len(decision.Options) != 2 {
		t.Fatalf("len(Decision.Options) = %d, want 2", len(decision.Options))
	}
...
func TestValidate_Decision_Valid(t *testing.T) {
	formula := &Formula{
		Formula: "mol-valid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	if err := formula.Validate(); err != nil {
		t.Errorf("Validate failed for valid decision: %v", err)
	}
}

func TestValidate_Decision_MissingPrompt(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
...
func TestValidate_Decision_TooFewOptions(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	err := formula.Validate()
	if err == nil {
		t.Error("expected validation error for too few options")
	}
	if !strings.Contains(err.Error(), "at least 2 options") {
		t.Errorf("expected 'at least 2 options' error, got: %v", err)
	}
}

func TestValidate_Decision_MissingOptionID(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	err := formula.Validate()
	if err == nil {
		t.Error("expected validation error for missing option id")
	}
...
func TestValidate_Decision_MissingOptionLabel(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	err := formula.Validate()
	if err == nil {
		t.Error("expected validation error for missing option label")
	}
...
func TestValidate_Decision_DuplicateOptionID(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
		Steps: []*Step{
			{
				...
				Decision: &DecisionConfig{
					Prompt: "Choose:",
					Options: []DecisionOption{
						{ID: "same", Label: "Option A"},
						{ID: "same", Label: "Option B"},
					},
				},
			},
		},
	...
	err := formula.Validate()
	if err == nil {
		t.Error("expected validation error for duplicate option id")
	}
...
func TestValidate_Decision_InvalidDefault(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	if !strings.Contains(err.Error(), "does not match any option id") {
		t.Errorf("expected 'does not match any option id' error, got: %v", err)
	}
}

func TestValidate_Decision_ValidDefault(t *testing.T) {
	formula := &Formula{
		Formula: "mol-valid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	if err := formula.Validate(); err != nil {
		t.Errorf("Validate failed for valid decision with default: %v", err)
	}
}

func TestParse_DecisionInChildStep(t *testing.T) {
	jsonData := `{
  "formula": "mol-nested",
  "version": 1,
  "type": "workflow",
  "steps": [
	...
	if child.Decision == nil {
		t.Fatal("Child decision is nil")
	}
	if child.Decision.Prompt != "Select approach:" {
		t.Errorf("Child Decision.Prompt = %q, want 'Select approach:'", child.Decision.Prompt)
	}
	if len(child.Decision.Options) != 2 {
		t.Errorf("len(Child Decision.Options) = %d, want 2", len(child.Decision.Options))
	}
}

func TestValidate_Decision_InChildInvalid(t *testing.T) {
	formula := &Formula{
		Formula: "mol-invalid",
		Version: 1,
		Type:    TypeWorkflow,
	...
	err := formula.Validate()
	if err == nil {
		t.Error("expected validation error for invalid child decision")
	}
	if !strings.Contains(err.Error(), "at least 2 options") {
		t.Errorf("expected 'at least 2 options' error, got: %v", err)
	}
...
func TestParse_DecisionMinimal(t *testing.T) {
  ...
  "steps": [
	...
	if decision.Default != "" {
		t.Errorf("Decision.Default = %q, want empty", decision.Default)
	}
	if decision.Timeout != "" {
		t.Errorf("Decision.Timeout = %q, want empty", decision.Timeout)
	}
}
