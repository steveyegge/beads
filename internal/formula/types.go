...
func validateDecision(dc *DecisionConfig, errs *[]string, prefix string) {
	...
	for i, opt := range dc.Options {
		...
		if opt.ID == "" {
			*errs = append(*errs, fmt.Sprintf("%s: id is required", optPrefix))
			...
			if optionIDs[opt.ID] {
				*errs = append(*errs, fmt.Sprintf("%s: duplicate id %q", optPrefix, opt.ID))
			}
			optionIDs[opt.ID] = true

			if dc.Default == opt.ID {
				defaultFound = true
			}
		}
	...
	if dc.Default != "" && !defaultFound {
		*errs = append(*errs, fmt.Sprintf("%s.decision: default %q does not match any option id", prefix, dc.Default))
	}
}
