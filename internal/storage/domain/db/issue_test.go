...
func (s *testSuite) issueInsertIdempotent() {
	...
	out, err := r.Get(s.Ctx(), "bd-test-dup")
	s.Require().NoError(err)
...
func (s *testSuite) issueInsertComputesHash() {
	...
	out, err := r.Get(s.Ctx(), "bd-test-hash")
	s.Require().NoError(err)
...
func (s *testSuite) issueInsertBatchAll() {
	...
	got, err := r.GetByIDs(s.Ctx(), []string{"bd-batch-1", "bd-batch-2", "bd-batch-3"})
	s.Require().NoError(err)
	s.Len(got, 3)
}
...
func (s *testSuite) issueInsertBatchStopsOnError() {
	...
	got, err := r.GetByIDs(s.Ctx(), []string{"bd-stop-1", "bd-stop-3"})
	s.Require().NoError(err)
...
func (s *testSuite) issueUpdateAllowedFields() {
	...
	s.Require().NoError(r.Update(s.Ctx(), "bd-upd-1", updates, "tester"))

	out, err := r.Get(s.Ctx(), "bd-upd-1")
	s.Require().NoError(err)
...
func (s *testSuite) issueUpdateRejectsUnknownFields() {
	...
	err := r.Update(s.Ctx(), "bd-upd-bad", map[string]any{"id": "rename-attempt"}, "tester")
	s.Require().Error(err)
...
func (s *testSuite) issueUpdateMissingID() {
	r := s.issueRepo()
	err := r.Update(s.Ctx(), "bd-does-not-exist", map[string]any{"title": "x"}, "tester")
	s.Require().Error(err)
...
func (s *testSuite) issueUpdateEmpty() {
	...
	s.Require().NoError(r.Update(s.Ctx(), "bd-upd-empty", nil, "tester"))
}
...
func (s *testSuite) issueUpdateStatusType() {
	...
	out, err := r.Get(s.Ctx(), "bd-upd-status")
	s.Require().NoError(err)
...
func (s *testSuite) issueUpdateNormalizesTimestamp() {
	...
	s.Require().NoError(r.Update(s.Ctx(), "bd-upd-tz", map[string]any{"due_at": due}, "tester"))

	out, err := r.Get(s.Ctx(), "bd-upd-tz")
	s.Require().NoError(err)
...
func (s *testSuite) issueGetMissing() {
	_, err := s.issueRepo().Get(s.Ctx(), "bd-no-such-id")
	s.Require().Error(err)
	s.True(errors.Is(err, sql.ErrNoRows), "expected sql.ErrNoRows, got %v", err)
}

func (s *testSuite) issueGetEmptyID() {
	_, err := s.issueRepo().Get(s.Ctx(), "")
	s.Require().Error(err)
}

func (s *testSuite) issueGetByIDsEmpty() {
	out, err := s.issueRepo().GetByIDs(s.Ctx(), nil)
	s.Require().NoError(err)
	s.Nil(out)
}
...
func (s *testSuite) issueGetByIDsPartial() {
	...
	out, err := r.GetByIDs(s.Ctx(), []string{"bd-pres-1", "bd-pres-2", "bd-missing"})
	s.Require().NoError(err)
	s.Len(out, 2)
...
func (s *testSuite) issueSearchAll() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{})
	s.Require().NoError(err)
...
func (s *testSuite) issueSearchByStatus() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{Status: &closedStatus, IDPrefix: "bd-stat-"})
	s.Require().NoError(err)
	s.Len(out, 1)
...
func (s *testSuite) issueSearchByIssueType() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{IssueType: &bugType, IDPrefix: "bd-type-"})
	s.Require().NoError(err)
	s.Len(out, 1)
...
func (s *testSuite) issueSearchByIDPrefix() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{IDPrefix: "bd-pfx-"})
	s.Require().NoError(err)
	s.Len(out, 2)
}
...
func (s *testSuite) issueSearchByIDs() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{IDs: []string{"bd-ids-1", "bd-ids-3"}})
	s.Require().NoError(err)
	s.Len(out, 2)
}
...
func (s *testSuite) issueSearchLimit() {
	...
	out, err := r.Search(s.Ctx(), types.IssueFilter{Limit: 3, IDPrefix: "bd-lim-"})
	s.Require().NoError(err)
	s.Len(out, 3)
}
