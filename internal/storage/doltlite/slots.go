...
func (s *DoltliteStore) SlotSet(ctx context.Context, issueID, key, value, actor string) error {
...
func (s *DoltliteStore) SlotGet(ctx context.Context, issueID, key string) (string, error) {
	issue, err := s.GetIssue(ctx, issueID)
	if err != nil {
		return "", fmt.Errorf("getting issue %s: %w", issueID, err)
	}
...
func (s *DoltliteStore) SlotClear(ctx context.Context, issueID, key, actor string) error {
	issue, err := s.GetIssue(ctx, issueID)
	if err != nil {
		return fmt.Errorf("getting issue %s: %w", issueID, err)
	}
