...
func TestGetReadyWork_IncludeEphemeralPropagatesWispSearchError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "get ready work: fetch issues") || !strings.Contains(err.Error(), "wisps") {
		t.Fatalf("expected ready-work wisp hydration error context, got %v", err)
	}
}
