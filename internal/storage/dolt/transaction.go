...
func insertIssueTx(ctx context.Context, tx *sql.Tx, issue *types.Issue) error {
	_, err := tx.ExecContext(ctx, `
		INSERT INTO issues (
			...
			sender, ephemeral, wisp_type, pinned, is_template, crystallizes, spec_id, spec_changed_at
		) VALUES (
			?, ?, ?, ?, ?, ?, ?,
			?, ?, ?, ?, ?,
			?, ?, ?, ?, ?,
	...
	`,
		...
		issue.Sender, issue.Ephemeral, string(issue.WispType), issue.Pinned, issue.IsTemplate, issue.Crystallizes, issue.SpecID, issue.SpecChangedAt,
	)
	return err
}
