...
func TestUpdateIssue(t *testing.T) {
	...
	err = store.UpdateIssue(ctx, issue.ID, specUpdates, "test-user")
	if err != nil {
		t.Fatalf("UpdateIssue (spec_id) failed: %v", err)
	}
	updated, err = store.GetIssue(ctx, issue.ID)
	if err != nil {
		t.Fatalf("GetIssue failed: %v", err)
	}
	if updated.SpecID != "specs/auth/login.md" {
		t.Errorf("SpecID not updated: got %v, want specs/auth/login.md", updated.SpecID)
	}
	...
	err = store.UpdateIssue(ctx, issue.ID, specUpdates, "test-user")
	if err != nil {
		t.Fatalf("UpdateIssue (clear spec_id) failed: %v", err)
	}
	updated, err = store.GetIssue(ctx, issue.ID)
	if err != nil {
		t.Fatalf("GetIssue failed: %v", err)
	...
	if updated.SpecID != "" {
		t.Errorf("SpecID not cleared: got %v, want empty", updated.SpecID)
	}
}
