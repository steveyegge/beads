  ...
  bd dep add bd-abc bd-xyz         # Same as above (bd-abc depends on bd-xyz)`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if blocksID != "" {
			...
			// Add the dependency via daemon RPC
			...
			if _, err := daemonClient.AddDependency(depArgs); err != nil {
...
Examples:
  ...
  bd dep list gt-abc --direction=up -t tracks  # Show what tracks gt-abc (convoy tracking)`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Ensure store is available for direct queries (dep list needs storage access)
		if store == nil {
		...
		if direction == "up" {
			issues, listErr = store.GetDependentsWithMetadata(ctx, fullID)
		} else {
			issues, listErr = store.GetDependenciesWithMetadata(ctx, fullID)
		}
		if listErr != nil {
			FatalErrorRespectJSON("%v", listErr)
		}
...
var depRemoveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Remove dependency via daemon RPC
...
Examples:
  ...
  bd dep tree gt-0iqq --depth=3          # Limit to 3 levels deep`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Ensure store is available for direct queries (dep tree needs storage access)
		if store == nil {
			if rpc.GetDaemonHost() != "" {
		...
		if direction == "both" {
			...
			tree, treeErr = store.GetDependencyTree(ctx, fullID, maxDepth, showAllPaths, direction == "up")
			if treeErr != nil {
				FatalErrorRespectJSON("%v", treeErr)
			}
		}
...
var depCyclesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		requireDaemon("dep cycles")

		// Ensure store is available for direct queries (dep cycles needs storage access)
		if store == nil {
			if rpc.GetDaemonHost() != "" {
...
func resolveExternalDependencies(ctx context.Context, issueID string, typeFilter string) []*types.IssueWithDependencyMetadata {
	...
	for _, dep := range deps {
		...
		if err != nil {
			if isVerbose() {
				fmt.Fprintf(os.Stderr, "[external-deps] failed to open target db at %s: %v\n", targetBeadsDir, err)
			}
