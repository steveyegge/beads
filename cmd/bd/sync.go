...
Use --merge to merge the sync branch back to main branch.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		// Check if BEADS_DIR points to an external repository (wg-arn fix)
...
// This is used to detect when BEADS_DIR points to a separate repository (wg-arn fix).
...
// Used when BEADS_DIR points to a different git repository than cwd (wg-arn fix).
