  ...
  bd human list --json`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if jsonOutput {
			...
			labelsMap, _ := mustAnnot(store).GetLabelsForIssues(ctx, issueIDs) // Best effort: labels are supplementary display info
...
Examples:
  ...
  bd human respond bd-123 -r "Approved, proceed with implementation"`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		labelsMap, _ := mustAnnot(targetStore).GetLabelsForIssues(ctx, []string{resolvedID}) // Best effort: labels are supplementary display info
		hasHumanLabel := false
