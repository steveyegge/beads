...
// Read-only commands open the store in read-only mode, preventing modifications
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if dbPath == "" {
			...
			if foundDB := beads.FindDatabasePath(); foundDB != "" {
				...
				// - config set/get for yaml-only keys: writes to config.yaml, not the database (GH#536)
