...
func withStorage(_ context.Context, store storage.Storage, _ string, _ time.Duration, fn storageExecutor) error { //nolint:unparam // params kept for call-site consistency during migration
	if store != nil {
		return fn(store)
	}
...
func formatIssueCompact(buf *strings.Builder, issue *types.Issue, labels []string, progressMap map[string]*types.EpicProgress, blockedBy, blocks []string) {
	labelsStr := ""
	if len(labels) > 0 {
	...
	if issue.Status == types.StatusClosed {
		...
		line := fmt.Sprintf("%s %s%s [P%d] [%s]%s%s - %s%s",
			...
			typeBadge, assigneeStr, labelsStr, issue.Title, depInfo)
		buf.WriteString(ui.RenderClosedLine(line))
		buf.WriteString("\n")
	} else {
