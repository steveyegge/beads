TASK: beads multi-repo hydration - cross-repo issue resolution not working. bd repo add + bd repo sync complete in kb-cli but bd show orch-go-ivtg.3 fails with 'no issue found'. Understand: (1) how hydration is supposed to work, (2) why cross-repo IDs don't resolve, (3) is this a bug or expected limitation?

🚨 CRITICAL - FIRST 3 ACTIONS:
You MUST do these within your first 3 tool calls:
1. Report via `bd comment orch-go-djpb "Phase: Planning - [brief description]"`
...
2. Run: `bd comment orch-go-djpb "Phase: Complete - [1-2 sentence summary of deliverables]"`
...
2. **SET UP investigation file:** Run `kb create investigation beads-multi-repo-hydration-cross` to create from template
   - This creates: `.kb/investigations/simple/YYYY-MM-DD-beads-multi-repo-hydration-cross.md`
   - This file is your coordination artifact (replaces WORKSPACE.md)
   - If command fails, report to orchestrator immediately
   - **IMPORTANT:** After running `kb create`, report the actual path via:
     `bd comment orch-go-djpb "investigation_path: /path/to/file.md"`
...
6. **CREATE SYNTHESIS.md:** Before completing, create `SYNTHESIS.md` in your workspace: /Users/dylanconlin/Documents/personal/beads/.orch/workspace/og-inv-beads-multi-repo-21dec/SYNTHESIS.md
...
You were spawned from beads issue: **orch-go-djpb**

**Use `bd comment` for progress updates instead of workspace-only tracking:**

```bash
# Report progress at phase transitions
bd comment orch-go-djpb "Phase: Planning - Analyzing codebase structure"
bd comment orch-go-djpb "Phase: Implementing - Adding authentication middleware"
bd comment orch-go-djpb "Phase: Complete - All tests passing, ready for review"

# Report blockers immediately
bd comment orch-go-djpb "BLOCKED: Need clarification on API contract"

# Report questions
bd comment orch-go-djpb "QUESTION: Should we use JWT or session-based auth?"
```
...
**Why beads comments:** Creates permanent, searchable progress history linked to the issue. Orchestrator can track progress across sessions via `bd show orch-go-djpb`.
...
1. Ensure SYNTHESIS.md is created and committed in your workspace.
2. `bd comment orch-go-djpb "Phase: Complete - [1-2 sentence summary]"`
3. `/exit`
